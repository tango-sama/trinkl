...
- Scheduled functions (`onSchedule`, timezone `Africa/Algiers`): `refreshAllParcels` at 00:00 refreshes every parcel that is not finished yet; `syncMetaInsights` at 03:00 pulls Meta ad insights.
...
- Tracking status is refreshed two ways, both through the same `refreshOrderStatus` helper: on demand (`getParcelStatus`) from the admin panel's per-order 🔄 button, and once nightly at 00:00 (`refreshAllParcels`) over every parcel that is not finished yet. A parcel is finished when `outcomeFromStatus` reads its stored status as `delivered`, `returned` or `cancelled` (the last covers deletion at the carrier) — those are never re-asked, so the nightly cost tracks parcels in flight, not orders ever placed. Nothing polls in a loop: one call per unfinished parcel per night, paced 350ms apart.
